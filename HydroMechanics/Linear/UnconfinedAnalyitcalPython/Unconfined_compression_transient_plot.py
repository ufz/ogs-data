...
data = pd.read_csv('OGS6_1000s_transient_t10s_r10.csv')
roots = np.array(
...
def plot_temp_ana(name):
    ...
    t = np.arange(10, 1010, 10)  # unit second (transient)
    #t = 1000# unit second

    tol = 1e-12  # tolerance to stop

    r = 1  # the location is at the outer radius
    ...
    u1 = np.zeros(len(t))
    for i in range(len(t)):
        u1[i] = r0 * (-epsilon0) * lambda_s / (HA + lambda_s) * r / r0
    ...
    for i in range(len(t)):
        ...
        while error > tol:
            ...
            a3 = sp.j1(roots[n] * r / r0)
            a4 = math.exp(-HA * K / (r0 ** 2) * (roots[n] ** 2) * t[i])
         #  u2[i] += (HA - lambda_s) / (
    ...
    ax2 = ax1.twinx()
    lns1 = ax1.plot(data.Time, data.displacement0, "o", mec="red", mfc="none", label='ogs6')
    lns2 = ax1.plot(t, u, "b--", label='analytical')
    lns3 = ax2.plot(t, u_error, "black", label='Error')
   ...
   # legend = ax1.legend(loc='upper left', shadow=True)
    ...
    plt.title('Transient Unconfined Compression')
